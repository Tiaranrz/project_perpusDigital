<?php 
  $uri = service('uri');
  $segment2 = $uri->getSegment(2); // dashboard, users, books, etc
...
<aside id="mainSidebar"
  ...
  <div class="flex-1 flex flex-col">
    <ul class="flex-1 flex flex-col gap-1 px-2">
      ...
      <li>
        <a class="flex items-center gap-2 px-4 py-2.5 rounded-xl font-semibold transition
          <?= $segment2 == 'users' ? 'bg-fuchsia-500 text-white shadow' : 'text-slate-700 hover:bg-gray-100' ?> "
          href="<?= base_url('admin/users/admin') ?>">
          <span class="flex items-center justify-center w-8 h-8 rounded-lg bg-gradient-to-tl from-purple-700 to-pink-500 shadow">
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  const toggleBtn = document.getElementById('sidebarToggleBtn'); // Pastikan id ini sama di navbar
