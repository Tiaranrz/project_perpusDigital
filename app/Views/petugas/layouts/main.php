<!DOCTYPE html>
<html lang="en">
<?= $this->include('petugas/layouts/header') ?>
<body class="m-0 font-sans text-base antialiased font-normal leading-default bg-gray-50 text-slate-500">
    <?= $this->include('petugas/layouts/sidebar') ?>
    ...
    <main class="ease-soft-in-out xl:ml-[250px] relative min-h-screen rounded-xl transition-all duration-200">
        <?= $this->include('petugas/layouts/navbar') ?>
        <div class="w-full px-6 py-6 mx-auto">
            <?= $this->renderSection('content') ?>
            <?= $this->include('petugas/layouts/footer') ?>
        </div>
    </main>
    <?= $this->include('petugas/layouts/script') ?>
</body>
</html>
