  ...
  <ol class="inline-flex items-center space-x-1 md:space-x-3">
    ...
    <li>
      <div class="flex items-center">
        ...
        <a href="<?= base_url('petugas/books') ?>" class="ml-1 text-gray-500 hover:text-fuchsia-600 transition">Manajemen Buku</a>
      </div>
    </li>
    <li>
      <div class="flex items-center">
        <i class="fa fa-chevron-right mx-2 text-slate-400"></i>
        <span class="ml-1 text-fuchsia-600 font-semibold"><?= isset($category) ? 'Edit Kategori' : 'Tambah Kategori' ?></span>
      </div>
    </li>
  </ol>
</nav>
...
<div class="mx-auto max-w-lg bg-white px-4 py-6 rounded-2xl shadow-lg border border-fuchsia-100 mb-8">
  <div class="text-center mb-5">
    <h2 class="text-lg font-bold text-fuchsia-700 inline-flex items-center gap-2 mb-1">
      ...
      <?= isset($category) ? 'Edit Kategori Buku' : 'Tambah Kategori Buku' ?>
    </h2>
  </div>
  ...
  <form action="<?= esc($form_action) ?>" method="post" class="space-y-4" autocomplete="off">
    ...
    <div>
      <label class="block mb-1 font-medium text-slate-700 text-sm">
        <i class="fa fa-tag text-fuchsia-400 mr-1"></i> Nama Kategori
      </label>
      <input name="nama" type="text" value="<?= old('nama', $category['nama'] ?? '') ?>"
        class="w-full px-3 py-2 border border-fuchsia-200 rounded focus:outline-none focus:border-fuchsia-400 text-sm"
        required>
    </div>
    <div>
      <label class="block mb-1 font-medium text-slate-700 text-sm">
        <i class="fa fa-info-circle text-fuchsia-400 mr-1"></i> Deskripsi (Opsional)
      </label>
      <textarea name="deskripsi" rows="2"
        class="w-full px-3 py-2 border border-fuchsia-200 rounded focus:outline-none focus:border-fuchsia-400 text-sm"><?= old('deskripsi', $category['deskripsi'] ?? '') ?></textarea>
    </div>
    <!-- Tombol -->
    <div class="flex flex-col sm:flex-row justify-end gap-2 mt-6">
      <a href="<?= base_url('petugas/books') ?>"
      ...
      <button class="w-full px-6 py-2 rounded-lg bg-fuchsia-600 text-white font-semibold shadow hover:bg-fuchsia-700 transition flex items-center justify-center gap-2 text-sm">
